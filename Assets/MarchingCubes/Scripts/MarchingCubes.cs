...
public class MarchingCubes
{
    ...
    private VoxelPoint[] _initPoints;
    private Mesh _mesh;
    private int[,,] _cubeIndexes;
    
    private readonly Vector3 zero = Vector3.zero;

    public MarchingCubes(VoxelPoint[,,] points, float isolevel, int seed)
    {
        _isolevel = isolevel;
    ...
    private void March(VoxelPoint[] points, int cubeIndex)
    {
    ...
    private int CalculateCubeIndex(VoxelPoint[] points, float iso)
    {
        int cubeIndex = 0;
    ...
    public Mesh CreateMeshData(VoxelPoint[,,] points)
    {
        ...
        for (int x = 0; x < points.GetLength(0)-1; x++)
        {
            for (int y = 0; y < points.GetLength(1)-1; y++)
            {
                for (int z = 0; z < points.GetLength(2)-1; z++)
                {
                    ...
                    March(GetVoxelPoints(x, y, z, points), cubeIndex);
                }
            }
        }
    ...
    private int[,,] GenerateCubeIndexes(VoxelPoint[,,] points)
    {
        for (int x = 0; x < points.GetLength(0)-1; x++)
        {
            for (int y = 0; y < points.GetLength(1)-1; y++)
            {
                for (int z = 0; z < points.GetLength(2)-1; z++)
                {
                    _initPoints = GetVoxelPoints(x, y, z, points);
